...
function drawBGTiled(ctx, stage, camX, camY) {
  ...
  // Stages with a single full-world background (like stage 0)
  const fullKey = 'stage' + stage + 'full';
  const fullImg = BG_IMG_RAW[fullKey];

  if (fullImg) {
    // Draw image at native-ish scale (fit to world, pick best crisp scale)
    const iw = fullImg.naturalWidth, ih = fullImg.naturalHeight;
    // Scale so image covers the world — pick the larger scale factor
    const fitScale = Math.max(WORLD_W / iw, WORLD_H / ih);
    const drawW = iw * fitScale, drawH = ih * fitScale;
    // Center the scaled image on the world
    const offX = (WORLD_W - drawW) / 2, offY = (WORLD_H - drawH) / 2;
    // Source rect from camera position
    ...
    // Check if AI tile exists for the current stage
    const aiTile = BG_IMG_RAW['stage' + stage];
    const tile = aiTile || getBGTile(stage);
    const ts = aiTile ? 640 : TILE_SIZE;

    // Dim the AI backgrounds slightly to maintain contrast with the game entities,
    // if using the raw image.
    if (aiTile) {
      ctx.save();
      // Apply a global darkening effect over the whole background
      ctx.globalAlpha = 0.6;
    }

    const sx = Math.floor(camX / ts) * ts;
    const sy = Math.floor(camY / ts) * ts;
    for (let x = sx; x < camX + W + ts; x += ts) {
      for (let y = sy; y < camY + H + ts; y += ts) {
        const dx = x - camX;
        const dy = y - camY;
        if (aiTile) {
          // Seamless mirroring — flip alternating tiles for perfect edge matching
          const tx = Math.floor(x / ts);
          const ty = Math.floor(y / ts);
          ctx.save();
          ctx.translate(dx + ts / 2, dy + ts / 2);
          ctx.scale(tx % 2 !== 0 ? -1 : 1, ty % 2 !== 0 ? -1 : 1);
    ...
    if (aiTile) {
      ctx.restore();
      // Stage-specific color atmosphere overlay
...
function drawPlayAreaBorder(ctx, stage, camX, camY) {
  ...
  ctx.fillRect(0, 0, W, T - rw / 2);
  ctx.fillRect(0, B + rw / 2, W, H - B - rw / 2);
  ctx.fillRect(0, T - rw / 2, L - rw / 2, B - T + rw);
  ctx.fillRect(R + rw / 2, T - rw / 2, W - R - rw / 2, B - T + rw);

  // 2) Ribbon — gradient band with inner glow
  ctx.save();
  const ac = pal.accent;

  // Helper: draw one ribbon edge
  function ribbon(x1, y1, x2, y2, nx, ny) {
    // nx,ny = normal direction (outward)
    const len = Math.hypot(x2 - x1, y2 - y1);
    if (len < 1) return;

    // Outer gradient fade (into dead zone)
    const gOut = ctx.createLinearGradient(
      x1 + nx * rw / 2, y1 + ny * rw / 2,
      x1 + nx * rw * 2, y1 + ny * rw * 2
    );
    gOut.addColorStop(0, ac + '18');
    gOut.addColorStop(1, 'transparent');
    ctx.fillStyle = gOut;
    ctx.beginPath();
    ctx.moveTo(x1 + nx * rw / 2, y1 + ny * rw / 2);
    ctx.lineTo(x2 + nx * rw / 2, y2 + ny * rw / 2);
    ctx.lineTo(x2 + nx * rw * 2, y2 + ny * rw * 2);
    ctx.lineTo(x1 + nx * rw * 2, y1 + ny * rw * 2);
    ctx.fill();

    // Inner gradient fade (into play zone)
    const gIn = ctx.createLinearGradient(
      x1 - nx * rw / 2, y1 - ny * rw / 2,
      x1 - nx * rw * 1.5, y1 - ny * rw * 1.5
    );
    gIn.addColorStop(0, ac + '10');
    gIn.addColorStop(1, 'transparent');
    ctx.fillStyle = gIn;
    ctx.beginPath();
    ctx.moveTo(x1 - nx * rw / 2, y1 - ny * rw / 2);
    ctx.lineTo(x2 - nx * rw / 2, y2 - ny * rw / 2);
    ctx.lineTo(x2 - nx * rw * 1.5, y2 - ny * rw * 1.5);
    ctx.lineTo(x1 - nx * rw * 1.5, y1 - ny * rw * 1.5);
    ctx.fill();

    // Core ribbon band
    const gCore = ctx.createLinearGradient(
      x1 - nx * rw / 2, y1 - ny * rw / 2,
      x1 + nx * rw / 2, y1 + ny * rw / 2
    );
    gCore.addColorStop(0, 'transparent');
    gCore.addColorStop(0.3, ac + '25');
    gCore.addColorStop(0.5, ac + '55');
    gCore.addColorStop(0.7, ac + '25');
  ...
  // Top, Bottom, Left, Right ribbons
  ribbon(L, T, R, T, 0, -1); // top edge — normal points up
  ribbon(L, B, R, B, 0, 1);  // bottom edge — normal points down
  ribbon(L, T, L, B, -1, 0); // left edge — normal points left
  ribbon(R, T, R, B, 1, 0);  // right edge — normal points right

  // 3) Corner ornaments — small glowing diamonds
  const cs = 6;
  ctx.fillStyle = ac;
  ctx.globalAlpha = 0.7;
