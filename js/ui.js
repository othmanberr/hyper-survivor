...
function updateHUD() {
    ...
    if (DOM.hpb) DOM.hpb.style.width = `${window._hpDisplay}%`;
    if (DOM.hpt) DOM.hpt.textContent = `${Math.ceil(P.hp)}/${P.maxHp}`;
    ...
    } else if (G.phase === 'waveIntro') {
        DOM.waveTimer.textContent = typeof formatTime === 'function' ? formatTime(Math.max(0, G.waveIntroTime)) : G.waveIntroTime;
        DOM.waveTimer.classList.remove('urgent');
        DOM.waveTimer.classList.add('warning');
    } else if (G.phase === 'boss' || G.phase === 'bossIntro') {
        DOM.waveTimer.textContent = 'BOSS';
        DOM.waveTimer.classList.remove('warning');
        DOM.waveTimer.classList.add('urgent');
    } else {
...
function buildWeaponRecap(containerId) {
    ...
    for (const w of P.weapons) {
      ...
      <div class="weapon-recap-item">
        ...
        <div class="weapon-recap-lvl">${lvlTag}</div>
      </div>
    `;
    }
}
...
function _wireSettings() {
    // Open buttons
    const btnMain = document.getElementById('btn-settings-main');
    if (btnMain) btnMain.addEventListener('click', () => openSettings('mm'));

    const btnPause = document.getElementById('btn-settings-pause');
    if (btnPause) btnPause.addEventListener('click', () => openSettings('pause-menu'));
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _wireSettings);
} else {
    _wireSettings();
}
